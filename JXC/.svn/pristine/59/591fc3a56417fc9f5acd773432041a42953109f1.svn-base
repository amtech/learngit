		...
		<form id="editform">
			<table cellpadding="12" align="center" width="100%">
				...
				<tr>
					...
					<td width="15%">SIM卡</td>
					<td width="15%"><input  type="text" name="sim_id" onblur="checkIsBind(this);" value="${equipment.sim_id }" style="background-color: white;" ></input></td>
				...
				function checkIsBind(id){
					...
					jQuery.ajax({
						...
						success:function(data){
							//alert(data);
							if(data==1){
								if(confirm("此设备已经绑定,确定要更新其绑定设备吗?")){
									$("#isbinding").val("1");
								}
							}
						}
