...
public class EquipmentCtl {
	...
	public String lists(Model model,HttpServletRequest request){
	     ...
	     String countername = "";
	     if(ce.getCounter_code()!=null){
	    	 countername = equipmentservice.queryCounterName(ce.getCounter_code());
	     }
	...
	public @ResponseBody String queryEquipmentList(String page,String rows,HttpServletRequest request){
		...
		for(EquipmentEntity ee:lists){
			...
			if("0001".equals(ee.getEquipment_type())){
				...
				InfoEntity ies = infoservice.queryInfoById(ee.getSim_id());
				if(ies != null){
					String dayrent = ies.getDay_rent();
					if(dayrent!=null){
						ee.setDay_rent(dayrent);
					}
				}
			}else{
			...
			if("".equals(ee.getSim_id()) || null == ee.getSim_id()){
				ee.setSim_id("未绑定");
			}else{
				ee.setSim_id("已绑定");
			}
...
//		String ee  =  JSONUtils.toJSONString(bd);
	...
	public @ResponseBody String queryWithCondition(EquipmentEntity equipmentEntity,String page,String rows,String countrys){
		...
		Map map = new HashMap();
		map.put("equipment", equipmentEntity);
	...
	public String uploadfile(@RequestParam("resourcefile") CommonsMultipartFile file,HttpServletRequest request,Model model,String counterid)throws Exception{
	        ...
	        if(file.getContentType().equalsIgnoreCase("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")){
	            ...
	            if(!queryIsDouble(equipmentnolist)){
	            	model.addAttribute("status","1");
					return "equipment/overthrough";
	            }
	            //先将数据添加到临时表，然后测试表中的数据
	            equipmentservice.batchInsertTemporatoryEqupment(equipmentlist);
	            List<String> tempeqptno = equipmentservice.queryEqptnoTemporatory();
	            for(String no : tempeqptno){
	            	Map map = new HashMap();
	            	map.put("no", no);
	            	int l = equipmentservice.queryIfDoubleData(map);
	            	if(l>0){
	            		model.addAttribute("status","1");
						return "equipment/overthrough";
	            	}
	        	...
	        	try{
	        	List<String> eq = new ArrayList<String>();//保存所有的设备号
	            ...
	            System.out.println("执行到检测XLS表中数据"+queryIsDouble(eq));
				if(!queryIsDouble(eq)){
					model.addAttribute("status","1");
					return "equipment/overthrough";
				}
	            ...
	            List<String> tempeqptnos = equipmentservice.queryEqptnoTemporatory();
	            System.out.println("========================="+tempeqptnos);
				for(String no : tempeqptnos){
	            	Map map = new HashMap();
	            	map.put("no", no);
	            	int l = equipmentservice.queryIfDoubleData(map);
	            	if(l>0){
	...
	public void downloadModel(HttpServletResponse response){
		InputStream in = this.getClass().getResourceAsStream("/equipment.xls");
        try {
