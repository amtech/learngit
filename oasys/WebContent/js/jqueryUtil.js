(function ($)
{
	...
	jqueryUtil.chgSkin=function(selectId,cookiesColor){
      ...
      //高级查询
		...
		//查询数据字典表
		jqueryUtil.getTextArr = function(type){
			var arr = new Array();
			$.ajax({
				type:'post',
				url:'baseAction!getDicText.action',
				data:"code="+type,
				async:false,
				dataType:'json',
		...
		//查询区域字典表
		jqueryUtil.getAreaTextArr = function(mainID){
			if(mainID==null){
				return;
			}
			var arr = new Array();
			$.ajax({
				type:'post',
				url:'baseAction!getAreaDicText.action',
				data:"code="+mainID,
				async:false,
				dataType:'json',
				success:function(iJson){
		...
		//回显类型
		jqueryUtil.showText = function(value,arr){
			var text = "";
			if(null==value||null==arr){
				return text;
			}
			for (var i in arr) {
				if(arr[i].dictCode == value){
					text += arr[i].dictName;
				}
			}
			return text;
	...
	/**
	 * form表单自动转化为json串，格式为{"aa":"1","bb":"2"}
	 */
