...
$(function(){
	...
	function saveTaskFunc(result,isSuccess){
		...
		$("#isSuccess").val(isSuccess);
    	 $.ajax({
    		type: "POST",
    		url:"consumablesApp/saveTaskConsumablesApp.do",
	   ...
	   <input type="hidden" id="isSuccess" name="isSuccess" value="" />
			<table class="table"  width="95%">
				<tr>
					...
					<th>申请人:</th>
					<td>
						<input id="account" name="account" type="text"  class="easyui-textbox"  disabled="disabled" />
					</td>
				</tr>
				<tr>
					<th>申请部门:</th>						
					<td>
						<input id="appDeptName" name="appDeptName" type="text"   class="easyui-textbox"  disabled="disabled"  />
					</td>
					<th>申请时间:</th>
					<td>
				...
				<tr>
					<td colspan="4">
				      ...
				      <a href="javascript:void(0)" id="save" class="easyui-linkbutton" iconCls="icon-save" onclick="saveTaskFunc('YingYeBuJingLiTongGuo',1);">审批通过</a>
				      <a href="javascript:void(0)" id="rset" class="easyui-linkbutton" onclick="saveTaskFunc('YingYeBuJingLiBoHui',0);">审批驳回</a>
				   </td>
				</tr>					
			</table>
		</form>
