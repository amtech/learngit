...
<script type="text/javascript">
			...
			columns : [ [ 
			              {field : 'appNO',title : '申请编号',width : 120,sortable:true,align:'center'},
							...
							{field : 'id',title : '查看附件',width :90,align:'center',formatter:function(value,row,index){
									return "<a href='javascript:void(0);' onclick='lookAttachment("+row.creator+");'>查看附件</a>" ;
							}}
			              ] ]
		}); 
	});
	...
	//申请意见表查看上传附件
	function lookAttachment(creator){
		checkAttachementDetail($row.appNo,creator);
	}
	...
	function microcreditOpinion(result,isSuccess){
		...
		$.ajax({
			url :"UseStampApp/saveTaskMgrResult.do",
			data : $("#useStampTaskForm").serialize(),
			async: false,
			type : "post",
...
<div data-options="region:'north',title:'North Title',split:true">
	<div style="width: 986px;height: 190px;overflow: auto;">
			<form id="useStampTaskForm" method="post">
				<input id="usaId" name="usaId" type="hidden"/><!-- 用章申请id -->
		...
		<div style="width: 980px;height:30px;">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton" onclick="microcreditOpinion('ShenQingChongTi','1');">用章申请重提</a>
			<a href="javascript:void(0);" class="easyui-linkbutton" onclick="microcreditOpinion('ShenQingCheXiao','3');">用章申请撤销</a>
		</div>
