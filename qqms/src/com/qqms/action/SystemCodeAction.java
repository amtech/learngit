...
public class SystemCodeAction extends BaseAction implements ModelDriven<SystemCode>
{
	...
	*/
	...
	{
		boolean flag = systemCodeService.isExistsCode(getModel());
		Json json = new Json();
		if (flag) {
			json.setMessage("该字典编码已存在，请更换!");
		} else{
			json=getMessage(systemCodeService.persistenceSystemCodeDig(getModel(),permissionName,codePid));
		}
	...
	*/
	...
	{
		OutputJson(systemCodeService.findSystemCodeByType(getModel().getDictCode()));
		return null;
	}
